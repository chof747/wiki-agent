...
The worker claims queued jobs in stable scanner discovery order and invokes a language-agnostic runner executable. The runner receives one Prompt Envelope JSON object on stdin containing the stripped prompt, original comment text, attached target page, and comment identity, and must emit exactly one finalized Response JSON object on stdout. Logs go to stderr. A non-zero exit or timeout without a valid finalized response is mapped to `UPDATE_FAILED`.
