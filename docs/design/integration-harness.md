...
- If saved harness state points at a host port that Docker can no longer bind, the harness must pick a fresh available port, update repo-local state, and continue using the same repo-scoped container name.
...
- `uv run wiki-agent-integration test` keeps the harness available for reuse after the pytest run.
- `uv run wiki-agent-integration ci-test` runs the same setup and integration pytest path, then always tears the harness down before returning.
...
env UV_CACHE_DIR=/private/tmp/uv-cache uv run wiki-agent-integration ci-test
```
...
- GitHub Actions and other one-shot environments should prefer `uv run wiki-agent-integration ci-test` so the harness is torn down even when integration pytest fails.
- On non-manual events where every changed path is within the docs-only allowlist (`docs/**`, `AGENTS.md`, `CONTEXT.md`, `config.example.toml`), the `CI / pytest` and `CI / integration` jobs are skipped at the job level instead of starting their runners and executing a no-op step. In GitHub Actions they appear as `skipped`, not as separately executed successful no-op jobs.
